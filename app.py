...
app = Flask(__name__)
app.secret_key = 'chave-secreta-supersegura'  # pode ser qualquer texto





# Render usa DATABASE_URL como variável de ambiente
...
def entrada():
    if request.method == 'POST':
        ...
        categoria = 'estéril' if tipo in estereis else 'não estéril'
        ...
        c.execute('''
        SELECT id FROM roupas WHERE tipo=? AND tamanho=? AND lote=? AND validade=?
        ...
        if result:
            c.execute('''
                UPDATE roupas SET quantidade = quantidade + ? WHERE id = ?
            ''', (quantidade, result[0]))
        else:
            c.execute('''
...
def saida():
    if request.method == 'POST':
        ...
        for roupa_id in selecionados:
            try:
                ...
                c.execute('UPDATE roupas SET quantidade = quantidade - ? WHERE id = ?', (quantidade, roupa_id))

                # Remove do banco se a quantidade zerou
    ...
    # GET → montar roupas agrupadas por tipo
    conn = conectar_postgres()
    c = conn.cursor()
